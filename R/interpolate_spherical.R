...
#' \insertCite{Lang2010;textual}{rcaiman}. For efficiency, interpolation runs on
...
#'   where the first four define geometry and `rr` is the value to
#'   interpolate.
...
#'   applied.
#' @param w numeric vector of length one or single-layer
#'   [terra::SpatRaster-class]. Weight assigned to `filling_source` in the blend
#'   with local estimates (see Eq. 6 in \insertCite{Lang2010;textual}{rcaiman}).
  ...
  .calculate_rr <- function(i) {
    ...
    if (switch(rule, any = any(m), all = all(m))) {
      ...
      if (is.vector(w)) {
        ...
        return(.eq6(w_k, sky_points[, "rr"], w[u][,], filling_source[u][,]))
      }
    } else {
